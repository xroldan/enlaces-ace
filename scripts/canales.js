...
function copiarListaM3U() {
  // Calcular la URL completa del archivo M3U relativa a la página actual
  const urlBase = window.location.href.substring(0, window.location.href.lastIndexOf('/') + 1);
  const m3uUrl = `${urlBase}data/local.m3u`;

  // Copiar al portapapeles
  if (navigator.clipboard && window.isSecureContext) {
    navigator.clipboard.writeText(m3uUrl)
      .then(() => mostrarToast('URL de la lista copiada! Pégala en VLC', 'success'))
      .catch(err => {
